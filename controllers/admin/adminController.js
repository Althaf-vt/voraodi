...
const loadDashboard = async (req, res, next) => {
    try {
        ...
        const filter = {
            period: req.query.period || req.session.filter?.period || 'monthly',
            startDate: req.query.startDate || req.session.filter?.startDate || null,
            endDate: req.query.endDate || req.session.filter?.endDate || null,
        ...
        if (filter.period === 'custom') {
            if (!filter.startDate || !filter.endDate || 
                !moment(filter.startDate).isValid() || !moment(filter.endDate).isValid()) {
                filter.period = 'monthly';
                filter.startDate = null;
                filter.endDate = null;
            } else if (moment(filter.endDate).isBefore(filter.startDate)) {
...
// Sales Report - Simplified to just handle filter changes
const saleReport = async (req, res, next) => {
    try {
        ...
        res.json({
            ...
            redirect: `/admin?period=${period}${startDate ? `&startDate=${startDate}` : ''}${endDate ? `&endDate=${endDate}` : ''}&page=${page}&limit=${limit}`
        });
    } catch (error) {
        res.status(500).json({ success: false, message: `Failed to apply filter: ${error.message}` });
    }
};

// Enhanced Get Sales Data function
const getSalesData = async (filter) => {
    const { period, startDate, endDate, page, limit } = filter;
    let match = { status: { $ne: 'Cancelled' } }; // Only include non-cancelled orders
    ...
    try {
        ...
        const formattedSalesReport = salesReport.map(order => ({
            ...
            orderDate: order.createdOn ? moment(order.createdOn).format('DD/MM/YYYY') : 'N/A',
        ...
        // Get all orders for charts and summary (without pagination)
        ...
        // Calculate summary
        const summary = {
            salesCount: allOrders.length,
            orderAmount: allOrders.reduce((sum, order) => sum + (order.finalAmount || 0), 0),
            discount: allOrders.reduce((sum, order) => sum + (order.discount || 0), 0)
        };

        // Generate income data for charts based on period
        const incomeData = generateIncomeData(allOrders, period, startDate, endDate);
        ...
        allOrders.forEach(order => {
            order.orderedItems.forEach(item => {
                ...
                if (!categorySales[categoryName]) {
                    categorySales[categoryName] = { totalSold: 0, totalRevenue: 0 };
                }
                categorySales[categoryName].totalSold += (item.quantity || 0);
                categorySales[categoryName].totalRevenue += revenue;
            });
        });
        ...
        return {
            totalSale: summary.orderAmount,
            totalOrders,
            totalCustomers,
            totalIncome: summary.orderAmount - summary.discount,
            summary,
...
// Helper function to generate income data for charts
function generateIncomeData(orders, period, startDate, endDate) {
    const incomeData = [];
    
    if (orders.length === 0) return incomeData;
    ...
    if (period === 'daily') {
        ...
        orders.forEach(order => {
            ...
            hourlyData[hour] = (hourlyData[hour] || 0) + income;
        });
        
        Object.entries(hourlyData).forEach(([hour, income]) => {
            incomeData.push({ date: hour, income });
        });
    } 
    else if (period === 'weekly' || (period === 'custom' && moment(endDate).diff(moment(startDate), 'days') <= 7)) {
        // Group by day
        const dailyData = {};
        const start = period === 'custom' ? moment(startDate) : moment().startOf('week');
        const end = period === 'custom' ? moment(endDate) : moment().endOf('week');
        ...
        Object.entries(dailyData).forEach(([date, income]) => {
            incomeData.push({ date, income });
        });
    }
    else if (period === 'monthly' || (period === 'custom' && moment(endDate).diff(moment(startDate), 'days') <= 31)) {
        // Group by day
        const dailyData = {};
        const start = period === 'custom' ? moment(startDate) : moment().startOf('month');
        const end = period === 'custom' ? moment(endDate) : moment().endOf('month');
        const daysDiff = end.diff(start, 'days');
        ...
        const monthlyData = {};
        const start = period === 'custom' ? moment(startDate).startOf('month') : moment().startOf('year');
        const end = period === 'custom' ? moment(endDate).endOf('month') : moment().endOf('year');
        ...
        Object.entries(monthlyData).forEach(([month, income]) => {
            incomeData.push({ date: month, income });
        });
    }
...
// Download PDF and Excel functions remain mostly the same but use the improved getSalesData
